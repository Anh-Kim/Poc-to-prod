...
class BaseTextCategorizationDataset:
    ...
    def get_index_to_label_map(self):
        ...
        """
        # TODO: CODE HERE
        # dict : convert list of tuple to dict
        # tuple (index, label)
        # return dict(enumerate(self._get_label_list()))
        # labels = ['a', 'b', 'c']
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        #expected = ['post_id', 'tag_name', 'tag_id', 'tag_position', 'title']
        #arr_col = np.array(column)
        #arr_exp = np.array(expected)
        #assert np.array_equal(arr_col, arr_exp)

        assert list(data.columns.tolist()) == ['post_id', 'tag_name', 'tag_id', 'tag_position', 'title']
        ...
        def filter_tag_position(position):
            def filter_function(df):
                ...
                """
                # TODO: CODE HERE
                df = df.loc[df['tag_position'] == position]
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                # TODO: CODE HERE
                #frequency = df.tag_name.value_counts()
                #df = df[df.tag_name.isin(frequency.index[frequency.gt(min_samples_per_label)])]
                #DataFrame shape mismatch
                #[left]:  (0, 5)
                #[right]: (1, 5)
                df_tag_filter = df.groupby('tag_name').filter(lambda x: len(x) >= min_samples_per_label)
                return df_tag_filter
        ...
        # TODO: CODE HERE
        pre_processed_dataset = data.pipe(filter_tag_position(0)).pipe(
            filter_tags_with_less_than_x_samples(min_samples_per_label))
