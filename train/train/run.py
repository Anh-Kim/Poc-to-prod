...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    #def __init__(self, filename, batch_size, train_ratio=0.8, min_samples_per_label=100, preprocess_text=lambda x: x):
    ...
    model = Sequential()
    model.add(Dense(units=train_conf['dense_dim'], activation="relu", input_shape=(768,))), #dense layer relu
        # units: Positive integer, dimensionality of the output space
    model.add(Dense(units=dataset.get_num_labels(), activation="softmax")) #output layer multi-classification problem
    ...
    # Convert train_history to a dictionary
    train_history_dict = {
        'epoch': train_history.epoch,
        'history': train_history.history
    }
    # Serialize train_history dictionary to a JSON string
    train_history_json = json.dumps(train_history_dict)
    # Save train_history JSON string to a file
    with open(os.path.join(artefacts_path, 'train_history.json'), 'w') as f:
        f.write(train_history_json)
